...
export async function tablesGeneratorFn(knexInstance: Knex): Promise<GenericObject> {
...
`;
  ...
  for (let i = 0; i < tableData.length; i++) {
    ...
    const obj = {
      ...
      columns: columnNames,
      relations: [],
      modelClassType: capitalizeFirstLetter(snakeToCamel(table.name)),
    };
  ...
  // TODO this type is wrong. Fix it
  const result: string[] = [];
